...
| `POST /classify` | `image` | `{ category, subCategory, confident }` — best-guess taxonomy match from the image alone (GPT-4o vision), `category`/`subCategory` null when `confident` is false. Used by Jewel Factory's similar-design search to pick a category for `/catalog` before it runs a raw query photo through the background-cleanup step — that page has no category picker, only a photo. NOT a description generator (see `/describe`, which goes the other way). **Jewel Factory's caller deliberately only forwards `category` to `/catalog`, never `subCategory`** (2026-08-07) — `/catalog`'s per-sub-category prompt branch means an AI-guessed subCategory that doesn't match what the manufacturer manually picked at Add Design time would visibly diverge the two cleanups of the same raw photo, drifting embeddings apart enough to miss real matches. If this endpoint's subCategory accuracy improves significantly later, that's a Jewel Factory-side decision to revisit, not something to change here. |
